...
def load_etf_data_to_duckdb(data_dir='data', db_path='etf.duckdb'):
    """
...
if __name__ == '__main__':
    load_etf_data_to_duckdb(db_path='etf.duckdb')

    # 可选: 验证数据是否已成功加载
    try:
        con_check = duckdb.connect('etf.duckdb')
        result = con_check.execute("SELECT * FROM main.etf_daily LIMIT 5").fetchdf()
        print("\n查询示例 (数据库中前5行数据):")
        print(result)
        con_check.close()
    except Exception as e:
